...
from ECBF import ECBF_control, discrete_ECBF, continuous_ECBF_version1, GP_ECBF_control
import dynamics_GP
from GP_learner import LEARNER
...
GP_agent = LEARNER(env)
dynamics_GP.build_GP_model(GP_agent)

for i_episode in itertools.count(1):
    episode_reward = 0
    episode_steps = 0
    i_episode_rewards = []
    if (GP_agent.firstIter == 1):
        pass
    else:
        GP_agent.GP_model_prev = GP_agent.GP_model.copy()
        dynamics_GP.build_GP_model(GP_agent)

    done = False
    state = env.reset()
    gp_states, gp_actions = [], []
    ...
    while not done:
        ...
        action_RL = action
        if GP_agent.firstIter == 1:
            f, g, x, std = dynamics_GP.get_GP_dynamics(GP_agent, state, action_RL)
        else:
            f, g, x, std = dynamics_GP.get_GP_dynamics_prev(GP_agent, state, action_RL)

        u_cbf, slack = GP_ECBF_control(action_RL, f, g, x, std)
        ...
        if done:
            path = {'observation': np.concatenate(gp_states).reshape((200,3)),
                    'action': np.concatenate(gp_actions)}

    if i_episode % 3 ==0:
        dynamics_GP.update_GP_dynamics(GP_agent, path)

    GP_agent.firstIter = 0
